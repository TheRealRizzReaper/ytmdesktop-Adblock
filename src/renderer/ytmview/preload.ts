...
(async function () {
  (
    await webFrame.executeJavaScript(`
      (function() {
        if (window.__adFetchPatched) return;
        window.__adFetchPatched = true;
        var _fetch = window.fetch;
        window.fetch = function(input, init) {
          var url = typeof input === "string" ? input : (input instanceof Request ? input.url : "");
          var isPlayer = url.includes("/youtubei/v1/player") || url.includes("/youtubei/v1/next") || url.includes("/youtubei/v1/browse");
          return _fetch.apply(this, arguments).then(function(res) {
            if (!isPlayer) return res;
            return res.clone().json().then(function(data) {
              function stripAds(obj) {
                if (!obj || typeof obj !== "object") return;
                var adKeys = ["adPlacements","adSlots","playerAds","adBreakHeartbeatParams","auxiliaryUi","adRenderer","adPlacementConfig","adBreakServiceRenderer","fullyAdFree","adSafetyReason","adInfoRenderer","adHoverTextButtonRenderer"];
                adKeys.forEach(function(k) { delete obj[k]; });
                Object.keys(obj).forEach(function(k) {
                  if (Array.isArray(obj[k])) obj[k].forEach(function(i) { stripAds(i); });
...
window.addEventListener("load", async () => {
  ...
  // Ad skipper using YTM internal store
  (
    await webFrame.executeJavaScript(`
      (function() {
        ...
        setInterval(function() {
          try {
            ...
            if (state && state.player && state.player.adPlaying) {
